...
- **分类筛选**：15 个静态分类 + 1 个实时分类 —— 打头的是 **🆕 新锐精选**（近两年发布且口碑出色：Docker DX、Astral ty、Kilo Code、Ruff、Console Ninja…）和 **🌱 新鲜上架**（见下）；实用的有语言支持、代码质量、Git 协作、界面主题、效率工具、前端开发、远程容器、AI 编程、数据接口、文档写作；不那么正经的有 **🐟 摸鱼炫技**（打字喷火的 Power Mode、盯盘神器韭菜盒子…）、**🔬 调试可视**、**📖 阅读翻译**、**🛠️ 工程配置**。点卡片上的分类标签也可快速筛选
